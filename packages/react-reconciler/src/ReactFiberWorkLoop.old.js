...
// 新的 Fiber 节点被创建，当 workInProgress 终于为空时，说明没有新的节点可以创建了，也就也意味着完成对整棵树 Fiber 树的构建
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  if (next === null) {
    ...
    // 每一个被创建出来的 Fiber 节点，都会一个一个挂载为最初的 workInProgress 节点的后代节点，从而构建出 workInProgress 树
    workInProgress = next;
  }
...
function completeUnitOfWork(unitOfWork: Fiber): void {
  ...
  do {
    ...
    if (siblingFiber !== null) {
      ...
      // 将 workInProgresss 赋值为当前节点的兄弟节点
    ...
    // 与上一步相辅相成，要求 workInProgress 与 completeWork 保持一致
...
function commitRootImpl(root, renderPriorityLevel) {
  do {
    ...
    // 触发 useEffect 回调与其他同步任务，由于这些任务可能触发新的渲染，所以这里要一遍遍历执行直到没有任务
    flushPassiveEffects();
