...
import {
  resolveDefaultProps
} from './ReactFiberLazyComponent.old';
import {
...
import {
  disableLogs,
  reenableLogs
} from 'shared/ConsolePatchingDev';
...
export function reconcileChildren(
  ...
  if (current === null) {
    ...
    // let's throw it out.
    // 若 current 不为 null，则进入 reconcileChildren 的逻辑
...
function updateMemoComponent(
  ...
  const currentChild = ((current.child: any):
    Fiber); // This is always exactly one child
  ...
  if (__DEV__) {
    if (workInProgress.type !== workInProgress.elementType) {
      ...
      if (outerMemoType.$$typeof === REACT_LAZY_TYPE) {
        ...
        const outerPropTypes = outerMemoType && (outerMemoType: any)
          .propTypes;
        if (outerPropTypes) {
          checkPropTypes(
            outerPropTypes,
  ...
  switch (resolvedTag) {
    ...
    case MemoComponent: {
      ...
      child = updateMemoComponent(
        ...
        workInProgress,
        Component,
        resolveDefaultProps(Component.type,
          resolvedProps), // The inner type can have defaults too
        updateLanes,
        renderLanes,
      );
      return child;
...
function validateSuspenseListNestedChild(childSlot: mixed, index: number) {
  if (__DEV__) {
    ...
    const isIterable = !isAnArray && typeof getIteratorFn(childSlot) ===
      'function';
...
function bailoutOnAlreadyFinishedWork(
  ...
  if (!includesSomeLane(renderLanes, workInProgress.childLanes)) {
    ...
    if (enableLazyContextPropagation && current !== null) {
      ...
      // the children.
      lazilyPropagateParentContextChanges(current, workInProgress,
        renderLanes);
      if (!includesSomeLane(renderLanes, workInProgress.childLanes)) {
        return null;
      }
...
function beginWork(
          ...
          if (state !== null) {
            if (enableSuspenseServerRenderer) {
              if (state.dehydrated !== null) {
                pushSuspenseContext(
                  workInProgress,
                  setDefaultShallowSuspenseContext(suspenseStackCursor
                    .current),
                );
  ...
  switch (workInProgress.tag) {
    ...
    case HostRoot: // 根节点进入这个逻辑
      return updateHostRoot(current, workInProgress, renderLanes);
    case HostComponent: // Dom 标签节点进入这个逻辑
      return updateHostComponent(current, workInProgress, renderLanes);
    case HostText: // 文本节点进入这个逻辑
    ...
    case SuspenseListComponent: {
      return updateSuspenseListComponent(current, workInProgress,
        renderLanes);
    }
    case ScopeComponent: {
      if (enableScopeAPI) {
    ...
    case LegacyHiddenComponent: {
      return updateLegacyHiddenComponent(current, workInProgress,
        renderLanes);
    }
    case CacheComponent: {
      if (enableCache) {
